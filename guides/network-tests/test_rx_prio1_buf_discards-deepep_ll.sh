...
# Usage: ./test_rx_prio1_buf_discards-deepep_ll.sh <namespace>
# Example: ./test_rx_prio1_buf_discards-deepep_ll.sh raj-network-debug
#
...
if [ $# -lt 1 ]; then
    ...
    echo "Example: $0 raj-network-debug"
    exit 1
fi
...
# Find all deepep-test-ll-lws pods and get their node names
all_pods=$(kubectl get pods -n "$NAMESPACE" -o jsonpath='{range .items[*]}{.metadata.name}{" "}{.spec.nodeName}{"\n"}{end}' | grep "^deepep-test-ll-lws")

# Print discovered pods
echo ""
echo "Discovered DeepEP Pods:"
echo "$all_pods"
...
echo ""
echo "Found ${#deepep_pods[@]} DeepEP pods (using first 2 for leader/worker test)"

# Print table showing deepep pods, node name, and networking debug pod name
echo ""
printf "%-25s %-18s %-40s\n" "DEEPEP POD" "NODE NAME" "NETWORKING DEBUG POD"
printf "%-25s %-18s %-40s\n" "-------------------------" "------------------" "----------------------------------------"
for i in "${!deepep_pods[@]}"; do
    printf "%-25s %-18s %-40s\n" "${deepep_pods[$i]}" "${node_names[$i]}" "${networking_debug_pods[$i]}"
done
...
for i in "${!networking_debug_pods[@]}"; do
    ...
    print_nic_counters "${deepep_pods[$i]}" "${node_names[$i]}" "$i" "BEFORE"
    print_pfc_counters "${deepep_pods[$i]}" "${node_names[$i]}" "$i" "BEFORE"
done
...
printf "%-25s %-18s %-18s %-18s\n" "-------------------------" "------------------" "------------------" "------------------"
for i in "${!deepep_pods[@]}"; do
    printf "%-25s %-18s %-18s %-18s\n" "${deepep_pods[$i]}" "${before_ecn_marked[$i]}" "${before_cnp_sent[$i]}" "${before_cnp_handled[$i]}"
done
...
for i in "${!networking_debug_pods[@]}"; do
    ...
    print_nic_counters "${deepep_pods[$i]}" "${node_names[$i]}" "$i" "AFTER"
    print_pfc_counters "${deepep_pods[$i]}" "${node_names[$i]}" "$i" "AFTER"
done
...
printf "%-25s %-18s %-18s %-18s\n" "-------------------------" "------------------" "------------------" "------------------"
for i in "${!deepep_pods[@]}"; do
    printf "%-25s %-18s %-18s %-18s\n" "${deepep_pods[$i]}" "${after_ecn_marked[$i]}" "${after_cnp_sent[$i]}" "${after_cnp_handled[$i]}"
done
...
echo "=============================================="
for i in "${!networking_debug_pods[@]}"; do
    print_nic_counter_diff "${deepep_pods[$i]}" "${node_names[$i]}" "$i"
    print_pfc_counter_diff "${deepep_pods[$i]}" "${node_names[$i]}" "$i"
done
...
echo "=================================================================================================================================="
printf "%-25s %-11s %-11s %-11s %-11s %-11s %-11s %-13s %-13s\n" \
...
for i in "${!deepep_pods[@]}"; do
    ...
    if [ $pod_skipped -gt 0 ]; then
        printf "%-25s %-11d %-11d %-11d %-11d %-11d %-11d %-13d %-13d (%d skip)\n" \
               "${deepep_pods[$i]}" "$pod_prio0_diff" "$pod_prio0_disc_diff" "$pod_prio1_diff" "$pod_prio1_disc_diff" "$pod_prio5_diff" "$pod_prio5_disc_diff" "$pod_phy_diff" "$pod_rdma_diff" "$pod_skipped"
    else
        printf "%-25s %-11d %-11d %-11d %-11d %-11d %-11d %-13d %-13d\n" \
               "${deepep_pods[$i]}" "$pod_prio0_diff" "$pod_prio0_disc_diff" "$pod_prio1_diff" "$pod_prio1_disc_diff" "$pod_prio5_diff" "$pod_prio5_disc_diff" "$pod_phy_diff" "$pod_rdma_diff"
    fi
done
...
for i in "${!deepep_pods[@]}"; do
    ...
    printf "%-25s %-10d %-10d %-10d %-10d %-12d %-12d %-12d %-12d\n" \
           "${deepep_pods[$i]}" "$pod_tx_p0" "$pod_rx_p0" "$pod_tx_p5" "$pod_rx_p5" "$pod_tx_p0_dur" "$pod_rx_p0_dur" "$pod_tx_p5_dur" "$pod_rx_p5_dur"
done
...
for i in "${!deepep_pods[@]}"; do
    ...
    printf "%-25s %-18d %-18d %-18d\n" "${deepep_pods[$i]}" "$ecn_diff" "$cnp_sent_diff" "$cnp_handled_diff"
done
